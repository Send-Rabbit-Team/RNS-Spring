...
public class BrokerService {
    ...
    private final int VALUE_MESSAGE_DURATION = 6 * 60;
    ...
    public String sendSmsMessage(BrokerMessageDto brokerMessageDto) {
        ...
        redisListRepository.rightPushAll(tmpKey, messageResultDtos, TMP_MESSAGE_DURATION);

        String valueKey ="message.value." + message.getId(); // tmp에서 TTL 만료됐을 경우 value값 꺼내오는 용도
        redisListRepository.rightPushAll(valueKey, messageResultDtos, VALUE_MESSAGE_DURATION);
...
//            }
//            redisListRepository.leftPop(valueKey); // value 값 제거
//
//            // 상태 값 저장
